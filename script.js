      ...
      produtos.forEach((p) => {
        corpoTabela.innerHTML += `
                    <tr>
                        ...
                        <td>${p.ESTOQUE}</td> 
                        <td>
                        <button onclick="preencherFormulario(${p.ID}, '${p.NOME}', '${p.CATEGORIA}', ${p.PRECO}, ${p.ESTOQUE})">
                        Editar
                        </button>
  ...
  fetch("http://localhost:3000/produto")
    ...
    .then((produtos) => {
      ...
      if (categoria === 'Acessórios') {
        tabelaId = 'tabelaAcessorios';
      } else {
        tabelaId = `tabela${categoria}`;
      }
      const corpoTabela = document.querySelector(`#${tabelaId} tbody`);
      corpoTabela.innerHTML = "";
...
function preencherFormulario(id, nome, categoria, preco, estoque) {
...
document.querySelectorAll('nav a').forEach(link => {
  link.addEventListener('click', (e) => {
    e.preventDefault();
    let categoria = link.textContent.replace('Consoles ', '');
    // Esconder todas as seções
    document.querySelectorAll('.console-section').forEach(section => {
      section.style.display = 'none';
    });
    // Mostrar a seção selecionada
    let sectionId = categoria.toLowerCase();
    if (categoria === 'Acessórios') {
